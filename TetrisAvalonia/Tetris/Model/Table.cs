...
namespace Tetris.Model
{
    public class Table
    {
        ...
        public void Rotate()
        {
            ...
            if (!IsValidPosition())
            {
                // Visszaforgatás (3x forgatás = 1x vissza)
                for (int i = 0; i < 3; i++)
                {
                    _currentShape.Rotate();
                }
        ...
        public int ClearFullLines()
        {
            ...
            for (int y = _height - 1; y >= 0; y--)
            {
                if (IsLineFull(y))
                {
                    ClearLine(y);
                    cleared++;
                    y++; // Mivel lecsúszott minden, újra ellenőrizni kell ezt a sort
                }
            }
            return cleared;
        }
            ...
            for (int x = 0; x < _currentShape.Size; x++)
            {
                for (int y = 0; y < _currentShape.Size; y++)
                {
                    if (_currentShape[x, y] != 0)
                    {
                        ...
                        {
                            // Itt feltételezem, hogy a Color egy int, vagy van mapping
                            // Ha a TetrisShape.Color még mindig System.Drawing.Color, 
                            // azt át kell írni int-re vagy enum-ra a Shape osztályban!
