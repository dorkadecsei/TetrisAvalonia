...
namespace Tetris.Model
{
    public class GameState
    {
        ...
        public Position CurrentPosition { get; set; } // Point -> Position
    ...
    public class GameModel
    {
        ...
        private readonly ITimer _timer; // Tetris.Persistence.ITimer helyett simán ITimer
        ...
        public GameModel(ITimer timer) : this(timer, 10, 20)
        {
        }

        public GameModel(ITimer timer, int width, int height)
        {
            _table = new Table(width, height);
            _timer = timer;
            // Itt majd castolni kell vagy beállítani, ha az ITimer támogatja az Interval-t
            _timer.Interval = 700;
        ...
        public void MoveLeft()
        {
            ...
            {
                if (_table.MoveLeft()) // Csak akkor frissítünk, ha sikeres volt
                    OnGameUpdated();
            }
        }
        ...
        public void MoveDown()
        {
            if (_gameStarted && !_isPaused && !IsGameOver)
            {
                // A Table.MoveDown visszatér false-al, ha leért (lockolta az alakzatot)
                if (!_table.MoveDown())
                {
        ...
        // Feltételezem, hogy ezeket a metódusokat megtartod, de figyelj a ShapeFactory-ra
        private static Shape? CloneShape(Shape original)
        {
            if (original == null) return null;

            // Itt hívod a Factory-t, feltételezve, hogy létezik és nem UI-függő
